...
router.get('/music/getSimilarArtists', auth, async (req, res) => {
    ...
    axios({
        ...
        res.send(result.data.similarartists.artist.map((artist) => artist.name))
    }).catch((e) => {
...
router.get('/music/getSimilarSongs', auth, async (req, res) => {
    ...
    axios({
        ...
        res.send(result.data.similartracks.track.map((track) => track.name))
    }).catch((e) => {
        res.status(400).send(e)  
    })
})
